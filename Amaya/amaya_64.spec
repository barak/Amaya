...
#BuildRoot:    /root/rpmbuild/BUILDROOT/%{name}-%{version}-%{release}
...
cd linux
export HOME=`pwd`
../configure --enable-system-raptor --prefix=$RPM_BUILD_ROOT/usr/share --datadir=$RPM_BUILD_ROOT/usr/share
#cp Options.orig Options
make depend
make
%install
cd linux
./script_install ./bin $RPM_BUILD_ROOT/usr/share $BUILDROOT/usr/bin 
# Gnome/Kde integration
./script_install_gnomekde ./bin $RPM_BUILD_ROOT/usr/share $BUILDROOT/usr/share
%files
%doc README
#/usr/share/pixmaps/
...
#/usr/share/Amaya/
%post
/bin/ln -f -s $RPM_BUILD_ROOT/usr/share/Amaya/wx/bin/amaya $RPM_BUILD_ROOT/usr/bin/amaya
%postun
rm -f $RPM_BUILD_ROOT/usr/bin/amaya
rm -f $RPM_BUILD_ROOT/usr/share/applications/Amaya.desktop
rm -f $RPM_BUILD_ROOT/usr/share/pixmaps/amaya.png
